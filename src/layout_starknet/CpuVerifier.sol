...
import "../common/CairoVerifierContract.sol";
import "../common/MemoryPageFactRegistry.sol";
import "../common/PublicMemoryOffsets.sol";
    ...
    PublicMemoryOffsets
{
    ...
    // Debug event
    event DebugInteractionElements(uint256 z, uint256 alpha);
    event DebugFactHash(bytes32 factHash, uint256 pageSize, uint256 memoryHash, uint256 prod);
    ...
    /**
     * @notice Returns the address of the MemoryPageFactRegistry used by this verifier
     * @return The address of the memory page fact registry
     */
    function getMemoryPageFactRegistry() external view returns (address) {
        return address(memoryPageFactRegistry);
    }

    /**
     * @notice Helper function to compute z and alpha for a given proof
     * This allows tests to register memory page facts with the correct values
     * @param proofParams The proof parameters
     * @param proof The proof data
     * @param publicInput The public input
     * @return z The first interaction element
     * @return alpha The second interaction element
     */
    function computeInteractionElements(
        uint256[] calldata proofParams,
        uint256[] calldata proof,
        uint256[] calldata publicInput
    ) external view returns (uint256 z, uint256 alpha) {
        ...
        z = ctx[getMmInteractionElements()];
        alpha = ctx[getMmInteractionElements() + 1];
    }
